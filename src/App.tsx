...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="page-head"><div><span className="eyebrow"><Sparkles size={15}/> ARAAK DEVELOPMENT COMMAND</span><h1>{titles[page][0]}</h1><p>{titles[page][1]}</p></div><div className="page-actions"><select value={period} onChange={e=>setPeriod(e.target.value)}><option>اليوم</option><option>هذا الأسبوع</option><option>هذا الشهر</option><option>الربع الثالث</option></select><button className="primary"><Radio size={17}/> تقرير مباشر</button></div></section>
...
function Overview({setPage}:{setPage:(p:NavKey)=>void}) {
  return <div className="dashboard">
    <div className="hero-status"><div><span><Zap size={16}/> موجز التنفيذ الآن</span><h2>العمل يسير وفق الخطة <em>وبزخم متصاعد</em></h2><p>تم إنجاز 23 مهمة هذا الأسبوع، ونشر 8 مواد، وتحويل 43 مهتماً إلى برامج الأكاديمية.</p></div><div className="hero-score"><div><b>86</b><small>/100</small></div><span>مؤشر صحة التشغيل</span></div></div>
    <div className="kpis">
      <Kpi icon={FileText} label="المحتوى المنشور" value="38" delta="+12 هذا الأسبوع" color="blue"/>
      <Kpi icon={TrendingUp} label="إجمالي الوصول" value="46.4K" delta="+28.6%" color="violet"/>
      <Kpi icon={Users} label="المهتمون المؤهلون" value="142" delta="+43 جديداً" color="green"/>
      <Kpi icon={CircleDollarSign} label="عائد البرامج" value="67.6K" delta="ريال سعودي" color="amber"/>
    </div>
...
function Content(){return <div className="dashboard"><div className="sub-kpis"><Mini label="الخطة الحالية" value="14 مادة" icon={CalendarDays}/><Mini label="منشور" value="8 مواد" icon={CheckCircle2}/><Mini label="قيد الإنتاج" value="4 مواد" icon={Activity}/><Mini label="الالتزام" value="92%" icon={Target}/></div><Card title="خط إنتاج المحتوى" sub="حزمة التشغيل التجريبية · 18–31 يوليو"><div className="content-table"><div className="table-head"><span>المادة</span><span>النوع والمنصة</span><span>موعد النشر</span><span>الحالة</span><span>الجودة</span></div>{contentItems.map((x,i)=><div className="table-row" key={i}><div><b>{x.title}</b><small>AR-{String(i+1).padStart(3,'0')}</small></div><div><b>{x.type}</b><small>{x.channel}</small></div><time>{x.date}</time><span className={`status s-${x.status.replace(' ','-')}`}>{x.status}</span><strong className="quality">{x.score}%</strong></div>)}</div></Card></div>}
